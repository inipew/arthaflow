...
// Mock DB for local development when D1 is not bound
const createMockDB = () => ({
  prepare: (sql: string) => ({
    bind: (...params: any[]) => ({
      all: async () => ({ results: [] }),
...
export async function getCategories(db: any, type?: 'income' | 'expense') {
  if (!db) {
...
export async function getTransactions(db: any, filters: {
  startDate?: string;
  endDate?: string;
  categoryId?: number;
  personName?: string;
...
export async function getStats(db: any) {
  if (!db) {
    return { income: 5000000, expense: 1750000, balance: 3250000 };
  }
  const stats = await db.prepare(`
    ...
    FROM transactions
  `).first();
  const income = (stats as any)?.total_income || 0;
  const expense = (stats as any)?.total_expense || 0;
  return { income, expense, balance: income - expense };
}

export async function getCategoryTotals(db: any) {
  if (!db) {
    return [
  ...
  const { results } = await db.prepare(`
    ...
    ORDER BY total DESC
  `).all();
  return results as { name: string; total: number; type: string }[];
}

export async function addTransaction(db: any, data: any) {
  if (!db) return;
  ...
  await db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).bind(
    id, data.type, data.amount, data.category_id, data.description || '', data.person_name || null, data.date
  ).run();
}

export async function deleteTransaction(db: any, id: string) {
  if (!db) return;
  await db.prepare('DELETE FROM transactions WHERE id = ?').bind(id).run();
}

export async function addCategory(db: any, name: string, type: 'income' | 'expense') {
  if (!db) return;
